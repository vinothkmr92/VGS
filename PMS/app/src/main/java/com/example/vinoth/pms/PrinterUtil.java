...
public class PrinterUtil {
    ...
    private void PrintBillWithMRP(ReceiptData rcptData) throws IOException {
        ...
        for(int k=0;k<rcptData.itemsCarts.size();k++){
            ...
            String qty = formater.format(rcptData.itemsCarts.get(k).Qty);
            ...
            Double amt = rcptData.itemsCarts.get(k).getPrice()* rcptData.itemsCarts.get(k).Qty;
            totalAmt+=amt;
            double mrpamt = mrpd* rcptData.itemsCarts.get(k).Qty;
            mrpTotalAmt+=mrpamt;
    ...
    private void PrintBillSeperate(ReceiptData rcptData,ItemsCart ic) throws IOException {
        ...
        posPtr.setAlignment(0);
            ...
            String qty = formater.format(ic.Qty);
            String price = formater.format(ic.getPrice());
            Double amt = ic.getPrice()*ic.Qty;
            billAmt+=amt;
            String amts=String.format("%.0f",amt);
            String line = "";
            if(Common.RptSize.equals("2")){
