        ...
        protected String doInBackground(String... params)
        {
            ...
            try
            {
                ...
                for(int k=0;k<rptData.itemsCarts.size();k++){
                    ...
                    String qty = formater.format(rptData.itemsCarts.get(k).getQty());

                    String price = formater.format(rptData.itemsCarts.get(k).getPrice());
                    Double amt = rptData.itemsCarts.get(k).getPrice()*rptData.itemsCarts.get(k).getQty();
                    billAmt+=amt;
                ...
                Double totalQty = rptData.itemsCarts.stream().mapToDouble(c->c.getQty()).sum();
